...
function guidanceText(env = process.env) {
  return [
    ...
    "- **Never hand the user a `http://localhost:<port>` URL** — localhost in THEIR browser is THEIR machine, not this container, so such a link never works for them. Share session-origin URLs instead: `oyren route add …` (preferred — shareable, clean paths), or the token-gated port proxy `<session-origin>/_oyren/port/<SESSION_TOKEN>/<port>/` (what the editor's Oyren Preview uses). Apps that emit absolute asset paths need a base path or a route.",
