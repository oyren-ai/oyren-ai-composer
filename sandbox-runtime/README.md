...
| `AGENT_KIND` | CLI coding agent to auto-launch in tmux (`claude-code`, …); absent ⇒ plain shell. `deepseek-harness` is the one kind that paints no TUI — its pane runs `oyren-dsh-web`, which serves dsh's browser UI on the Codespace's `dsh-<label>` hostname |
